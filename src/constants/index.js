import project1 from "../assets/projects/project-1.jpg";
import project2 from "../assets/projects/project-2.jpg";
import project3 from "../assets/projects/project-3.jpg";
import project4 from "../assets/projects/project-4.jpg";

export const HERO_CONTENT = `Hello! I'm Githma Manthinie, a passionate and dedicated front-end developer
 with a strong focus on creating stunning, user-friendly web interfaces. My journey in web development
  began with a fascination for how design and functionality can come together to create seamless user
 experiences. Over the years, I've honed my skills in various technologies and frameworks to bring ideas to life on the web.`;
export const ABOUT_TEXT = `I am a passionate and dedicated Front End Developer. With a keen eye for design
 and a strong technical foundation, I specialize in creating visually appealing and highly functional web applications. My journey 
 in web development began with a fascination for how websites work, and over the years, I've honed my skills to transform creative 
 ideas into digital reality.I thrive on challenges and enjoy solving complex problems with elegant solutions. My expertise includes
  HTML, CSS, JavaScript, and modern frameworks like React. I am constantly learning and staying up-to-date with the latest industry 
  trends to ensure that my work remains at the cutting edge of web development.`;
export const EXPERIENCES = [
  {
    year: "Feb 2020 - Nov 2020",
    role: "Internship trainee",
    company: "Sampath Bank.PLC",
    description: `As an intern at Sampath Bank PLC in the NSC department, I excelled in data entry and cheque clearance, ensuring accurate records and timely transactions. Demonstrated punctuality, effective teamwork, and consistently met deadlines, contributing to smooth departmental operations.`,
    technologies: ["Bank Management System"],
  },
  {
    year: "July 2023 - Jan 2024",
    role: "Trainee Quality Assurance Engineer",
    company: "EchonLabs.Pvt.Ltd",
    description: `As a Trainee Quality Assurance Engineer at EchonLabs Pvt. Ltd., I specialized in Manual Testing, API Testing, and Test Case Writing. I gained practical experience in web revamping for internal projects and contributed to the BaseNeo project, enhancing my skills in quality assurance and project delivery.`,
    technologies: ["TestLink", "Postman"],
  },
];

export const PROJECTS = [
  {
    title: "Weather App",
    image: project1,
    description:
      "A sleek, responsive weather forecasting app built with React and Vite, leveraging the OpenWeatherMap API for real-time weather data. It delivers accurate, up-to-date information with a modern, user-friendly interface.",
    technologies: ["HTML", "CSS", "React"],
  },
  {
    title: "To Do List",
    image: project2,
    description:
      "Introducing my TodoList App: a streamlined tool for managing tasks with ease. Leveraging basic CRUD operations, it enables users to create, read, update, and delete tasks efficiently, enhancing productivity and organization in everyday life.",
    technologies: ["React", "MongoDB", "Express", "NodeJS"],
  },
  {
    title: "Portfolio Website",
    image: project3,
    description:
      "Explore my React portfolio website crafted with Framer Motion for stunning animations and Tailwind CSS for sleek designs. Discover a showcase of projects, featuring responsive layouts and interactive elements that highlight my skills and creativity in web development.",
    technologies: ["React", "Framer Motion", "TailwindCSS"],
  },
  {
    title: "Hospital Management System",
    image: project4,
    description:
      "Experience efficiency in healthcare with my C-based Hospital Management System. This project optimizes hospital operations through robust management of patient records, appointments, and staff administration, ensuring streamlined workflows and improved patient care delivery.",
    technologies: ["C"],
  },
];

export const CONTACT = {
  address: "293/1, yahampath Mawatha, Godigamuwa, Mahargama ",
  phoneNo: "+94772282726",
  email: "githma00@gmail.com",
};
